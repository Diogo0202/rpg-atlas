...
export const veyraNpcSortLabels = { relation: "Relação atual", source: "Fonte documental", name: "Nome (A–Z)" } as const;
export type VeyraNpcSort = keyof typeof veyraNpcSortLabels;
...
export function sortVeyraNpcs(npcs: VeyraNpcRecord[], sort: VeyraNpcSort) {
  const sourceRank = (npc: VeyraNpcRecord) => npc.source?.url === kaneNpcSource.url ? 1 : 0;
  return [...npcs].sort((left, right) => {
    if (sort === "source") return sourceRank(left) - sourceRank(right) || left.sourcePage - right.sourcePage || left.name.localeCompare(right.name, "pt-BR");
    if (sort === "name") return left.name.localeCompare(right.name, "pt-BR");
    return veyraNpcCompendium.indexOf(left) - veyraNpcCompendium.indexOf(right);
  });
}
