...
import { V5_STORE, type V5SheetData } from "@shared/vampire-v5";
import { Armchair, Car, Check, Home, Shield, ShoppingBag, Sword } from "lucide-react";
...
function StoreContent() {
  ...
  const [category, setCategory] = useState<keyof typeof categories>("arma");
  ...
  const visible = V5_STORE.filter((item) => tab === "arsenal" ? item.category === "arma" || item.category === "armadura" || item.category === "equipamento" : item.category === category);
  ...
  <section className="mt-7 flex flex-wrap gap-2 border-b border-white/10 pb-4"><Button onClick={() => { setTab("arsenal"); setCategory("arma"); }} className={`rounded-none ${tab === "arsenal" ? "bg-[#b55b32] text-[#101211]" : "border border-white/15 bg-transparent text-[#eae3d5]"}`}><Sword className="mr-2 h-4 w-4" /> Arsenal</Button><Button onClick={() => { setTab("compras"); setCategory("equipamento"); }} className={`rounded-none ${tab === "compras" ? "bg-[#b55b32] text-[#101211]" : "border border-white/15 bg-transparent text-[#eae3d5]"}`}><ShoppingBag className="mr-2 h-4 w-4" /> Compras</Button>{tab === "compras" ? <div className="ml-0 flex flex-wrap gap-2 lg:ml-auto">{(["equipamento", "roupa", "moradia", "veiculo", "montaria"] as const).map((key) => <button key={key} onClick={() => setCategory(key)} className={`px-3 py-2 text-[9px] font-bold uppercase tracking-[0.12em] ${category === key ? "border-b-2 border-[#83a89a] text-[#a9c7bb]" : "text-[#b8b3a8]"}`}>{categories[key]}</button>)}</div> : <div className="ml-0 flex gap-2 lg:ml-auto">{(["arma", "armadura", "equipamento"] as const).map((key) => <button key={key} onClick={() => setCategory(key)} className={`px-3 py-2 text-[9px] font-bold uppercase tracking-[0.12em] ${category === key ? "border-b-2 border-[#83a89a] text-[#a9c7bb]" : "text-[#b8b3a8]"}`}>{categories[key]}</button>)}</div>}</section>
