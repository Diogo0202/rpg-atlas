...
import { BookOpen, Dices, FolderOpen, ImagePlus, Plus, RefreshCw, Save, ShieldAlert, Sparkles, Trash2, UserRound } from "lucide-react";
...
export default function CharacterArchive() {
  ...
  const rollPool = (attribute: AttributeKey, skill: SkillKey) => {
    const quickPool = Math.max(1, sheet.attributes[attribute] + sheet.skills[skill]);
  ...
  return (
    <section id="fichas" className="relative overflow-hidden bg-[#111312] text-[#eae3d5]">
      ...
      <div className="relative mx-auto max-w-[1440px] px-5 py-20 sm:px-8 lg:px-10 lg:py-28">
        ...
        <article className="relative mt-7 overflow-hidden border border-[#b55b32]/45 bg-[#171a18] p-6 text-[#eae3d5] sm:p-8"><div className="absolute inset-0 dossier-grid opacity-20" /><div className="relative flex flex-col justify-between gap-4 border-b border-white/10 pb-5 sm:flex-row sm:items-start"><div><div className="flex items-center gap-2 text-[10px] font-bold uppercase tracking-[0.17em] text-[#a9c7bb]"><ShieldAlert className="h-3.5 w-3.5" /> Integridade da ficha</div><h3 className="mt-3 font-serif text-[36px] leading-none">O corpo cobra registro.</h3></div><p className="max-w-[320px] text-[12px] leading-6 text-[#a6a397]">Marque cada caixa diretamente na ficha. As trilhas também são preservadas no cofre local do personagem.</p></div><div className="relative mt-6 grid gap-5 lg:grid-cols-2"><DamageTracker title="Vitalidade" subtitle="Dano físico e pressão da cena" marks={sheet.healthDamage} onCycle={(index) => cycleDamage("healthDamage", index)} /><DamageTracker title="Força de Vontade" subtitle="Tensão mental e integridade" marks={sheet.willpowerDamage} onCycle={(index) => cycleDamage("willpowerDamage", index)} /></div></article>
